@extends('layouts.app')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="space-y-6 bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <!-- Form Header -->
        <div class="border-b border-gray-100 pb-6">
            <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                <i class="fas fa-user-edit text-blue-600"></i>
                Edit Data Pasien
            </h1>
            <p class="text-gray-500 mt-1">Perbarui informasi data pasien dengan data yang valid</p>
        </div>

        <form action="{{ route('patients.update', $patient) }}" method="POST" class="space-y-8" id="patientForm">
            @csrf
            @method('PUT')

            <!-- Informasi Dasar -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- No BPJS -->
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                        <i class="fas fa-id-card text-blue-500"></i>
                        No BPJS
                        <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        <input type="text"
                               name="no_bpjs"
                               id="no_bpjs"
                               value="{{ old('no_bpjs', $patient->no_bpjs) }}"
                               class="w-full pl-4 pr-10 py-2.5 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200"
                               placeholder="Contoh: 0001234567890"
                               maxlength="13"
                               pattern="\d{13}"
                               title="Nomor BPJS harus 13 digit angka"
                               required>
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-gray-400">
                            <small>13 digit</small>
                        </div>
                    </div>
                    <p class="text-xs text-gray-500">Format: 13 digit angka tanpa spasi atau karakter khusus</p>
                    @error('no_bpjs')
                        <p class="text-sm text-red-600 flex items-center gap-1 mt-1">
                            <i class="fas fa-exclamation-circle"></i>
                            {{ $message }}
                        </p>
                    @enderror
                </div>

                <!-- Nama Lengkap -->
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                        <i class="fas fa-user text-blue-500"></i>
                        Nama Lengkap
                        <span class="text-red-500">*</span>
                    </label>
                    <input type="text"
                           name="name"
                           id="name"
                           value="{{ old('name', $patient->name) }}"
                           class="w-full px-4 py-2.5 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200"
                           placeholder="Masukkan nama lengkap"
                           pattern="[A-Za-z\s']+"
                           title="Nama hanya boleh berisi huruf, spasi, dan tanda petik"
                           required>
                    <p class="text-xs text-gray-500">Gunakan huruf dan spasi, hindari angka atau karakter khusus</p>
                    @error('name')
                        <p class="text-sm text-red-600 flex items-center gap-1 mt-1">
                            <i class="fas fa-exclamation-circle"></i>
                            {{ $message }}
                        </p>
                    @enderror
                </div>

                <!-- Tanggal Lahir -->
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                        <i class="fas fa-calendar-alt text-blue-500"></i>
                        Tanggal Lahir
                        <span class="text-red-500">*</span>
                    </label>
                    <input type="date"
                           name="birth_date"
                           id="birth_date"
                           value="{{ old('birth_date', $patient->birth_date) }}"
                           class="w-full px-4 py-2.5 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200"
                           max="{{ date('Y-m-d') }}"
                           required>
                    <p class="text-xs text-gray-500">Tanggal lahir tidak boleh lebih dari hari ini</p>
                    @error('birth_date')
                        <p class="text-sm text-red-600 flex items-center gap-1 mt-1">
                            <i class="fas fa-exclamation-circle"></i>
                            {{ $message }}
                        </p>
                    @enderror
                </div>

                <!-- Jenis Kelamin -->
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                        <i class="fas fa-venus-mars text-blue-500"></i>
                        Jenis Kelamin
                        <span class="text-red-500">*</span>
                    </label>
                    <select name="gender"
                            id="gender"
                            class="w-full px-4 py-2.5 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200"
                            required>
                        <option value="">Pilih Jenis Kelamin</option>
                        <option value="L" {{ old('gender', $patient->gender) == 'L' ? 'selected' : '' }}>Laki-laki</option>
                        <option value="P" {{ old('gender', $patient->gender) == 'P' ? 'selected' : '' }}>Perempuan</option>
                    </select>
                    @error('gender')
                        <p class="text-sm text-red-600 flex items-center gap-1 mt-1">
                            <i class="fas fa-exclamation-circle"></i>
                            {{ $message }}
                        </p>
                    @enderror
                </div>

                <!-- Detail Alamat -->
                <div class="md:col-span-2 space-y-6">
                    <div class="border-b border-gray-100 pb-2">
                        <h2 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                            <i class="fas fa-map-marker-alt text-blue-500"></i>
                            Detail Alamat
                        </h2>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Provinsi -->
                        <div class="space-y-2">
                            <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                                <i class="fas fa-map text-blue-500"></i>
                                Provinsi
                            </label>
                            <div class="relative">
                                <select id="provinceSelect" name="address_province"
                                        class="w-full pl-4 pr-10 py-3 rounded-lg border-gray-300 bg-white focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 appearance-none">
                                    <option value="">Pilih Provinsi</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-gray-500">
                                    <i class="fas fa-chevron-down text-sm"></i>
                                </div>
                            </div>
                        </div>

                        <!-- Kabupaten/Kota -->
                        <div class="space-y-2">
                            <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                                <i class="fas fa-city text-blue-500"></i>
                                Kabupaten/Kota
                            </label>
                            <div class="relative">
                                <select id="citySelect" name="address_city"
                                        class="w-full pl-4 pr-10 py-3 rounded-lg border-gray-300 bg-white focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 appearance-none" disabled>
                                    <option value="">Pilih Kabupaten/Kota</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-gray-500">
                                    <i class="fas fa-chevron-down text-sm"></i>
                                </div>
                            </div>
                        </div>

                        <!-- Kecamatan -->
                        <div class="space-y-2">
                            <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                                <i class="fas fa-building text-blue-500"></i>
                                Kecamatan
                            </label>
                            <div class="relative">
                                <select id="districtSelect" name="address_district"
                                        class="w-full pl-4 pr-10 py-3 rounded-lg border-gray-300 bg-white focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 appearance-none" disabled>
                                    <option value="">Pilih Kecamatan</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-gray-500">
                                    <i class="fas fa-chevron-down text-sm"></i>
                                </div>
                            </div>
                        </div>

                        <!-- Desa/Kelurahan -->
                        <div class="space-y-2">
                            <label class="text-sm font-medium text-gray-700 flex items-center gap-2">
                                <i class="fas fa-home text-blue-500"></i>
                                Desa/Kelurahan
                            </label>
                            <div class="relative">
                                <select id="villageSelect" name="address_village"
                                        class="w-full pl-4 pr-10 py-3 rounded-lg border-gray-300 bg-white focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 appearance-none" disabled>
                                    <option value="">Pilih Desa/Kelurahan</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-gray-500">
                                    <i class="fas fa-chevron-down text-sm"></i>
                                </div>
                            </div>
                        </div>

                        <!-- Kode Pos -->
                        <div class="space-y-2">
                            <label class="text-sm font-medium text-gray-700">Kode Pos</label>
                            <input type="text"
                                   name="postal_code"
                                   id="postal_code"
                                   value="{{ old('postal_code', $patient->postal_code) }}"
                                   class="w-full px-4 py-3 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 text-base"
                                   placeholder="12345"
                                   pattern="\d{5}"
                                   maxlength="5"
                                   inputmode="numeric"
                                   autocomplete="off"
                                   title="Kode pos harus 5 digit angka">
                            <p class="text-xs text-gray-500">Format: 5 digit angka</p>
                            @error('postal_code')
                                <p class="text-sm text-red-600 flex items-center gap-1 mt-1">
                                    <i class="fas fa-exclamation-circle"></i>
                                    {{ $message }}
                                </p>
                            @enderror
                        </div>

                        <!-- RT/RW -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <label class="text-sm font-medium text-gray-700">RT</label>
                                <input type="text"
                                       name="address_rt"
                                       id="address_rt"
                                       class="w-full px-4 py-3 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 text-base"
                                       placeholder="001"
                                       maxlength="3"
                                       inputmode="numeric"
                                       autocomplete="off"
                                       value="{{ old('address_rt', $patient->address_rt) }}"
                                       pattern="\d{1,3}"
                                       title="RT harus 1-3 digit angka">
                                <p class="text-xs text-gray-500">Format: 3 digit angka</p>
                            </div>
                            <div class="space-y-2">
                                <label class="text-sm font-medium text-gray-700">RW</label>
                                <input type="text"
                                       name="address_rw"
                                       id="address_rw"
                                       class="w-full px-4 py-3 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 text-base"
                                       placeholder="002"
                                       maxlength="3"
                                       inputmode="numeric"
                                       autocomplete="off"
                                       value="{{ old('address_rw', $patient->address_rw) }}"
                                       pattern="\d{1,3}"
                                       title="RW harus 1-3 digit angka">
                                <p class="text-xs text-gray-500">Format: 3 digit angka</p>
                            </div>
                        </div>

                        <!-- Detail Alamat -->
                        <div class="md:col-span-2 space-y-2">
                            <label class="text-sm font-medium text-gray-700">Detail Alamat (Nama Jalan/Gang/No. Rumah)</label>
                            <textarea name="address_street"
                                    id="address_street"
                                    rows="2"
                                    class="w-full px-4 py-2.5 rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200"
                                    placeholder="Contoh: Jl. Mawar No. 123 Gang Melati">{{ old('address_street', $patient->address_street) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Hidden field untuk gabungan alamat -->
            <input type="hidden" name="address" id="fullAddress" value="{{ old('address', $patient->address) }}">

            <!-- Action Buttons -->
            <div class="flex justify-end gap-4 pt-6 border-t border-gray-100">
                <a href="{{ route('patients.show', $patient) }}"
                   class="px-6 py-2.5 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition duration-200 flex items-center gap-2">
                    <i class="fas fa-arrow-left"></i>
                    Kembali
                </a>
                <button type="submit"
                        class="px-6 py-2.5 text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition duration-200 flex items-center gap-2">
                    <i class="fas fa-save"></i>
                    Simpan Perubahan
                </button>
            </div>
        </form>
    </div>
</div>

@push('scripts')
<!-- Tambahkan jQuery dan Select2 -->
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>

<script>
$(document).ready(function() {
    // Ambil data alamat dari string alamat lengkap
    const fullAddress = @json($patient->address ?? '');
    console.log('Full Address:', fullAddress);

    // Parse alamat lengkap jika ada
    let addressParts = {};
    if (fullAddress) {
        // Coba ekstrak RT/RW dari alamat lengkap
        const rtRwMatch = fullAddress.match(/RT\s*(\d{1,3})\s*\/?\s*RW\s*(\d{1,3})/i);
        if (rtRwMatch) {
            addressParts.rt = rtRwMatch[1].padStart(3, '0');
            addressParts.rw = rtRwMatch[2].padStart(3, '0');
        }

        // Coba ekstrak kode pos (5 digit di akhir string)
        const postalMatch = fullAddress.match(/\b(\d{5})\b/);
        if (postalMatch) {
            addressParts.postalCode = postalMatch[1];
        }

        // Coba ekstrak nama jalan (bagian sebelum RT/RW)
        if (rtRwMatch) {
            const streetPart = fullAddress.split('RT')[0].trim();
            if (streetPart) {
                addressParts.street = streetPart;
            }
        }
    }

    console.log('Parsed Address Parts:', addressParts);

    // Set nilai awal untuk input alamat
    if (addressParts.street) {
        $('#address_street').val(addressParts.street);
    }
    if (addressParts.rt) {
        $('#address_rt').val(addressParts.rt);
    }
    if (addressParts.rw) {
        $('#address_rw').val(addressParts.rw);
    }
    if (addressParts.postalCode) {
        $('#postal_code').val(addressParts.postalCode);
    }

    // Inisialisasi Select2
    $('#provinceSelect, #citySelect, #districtSelect, #villageSelect').select2({
        theme: 'classic',
        width: '100%',
        placeholder: {
            id: '',
            text: "Ketik untuk mencari..."
        },
        allowClear: true,
        language: {
            noResults: function() {
                return "Data tidak ditemukan";
            },
            searching: function() {
                return "Mencari...";
            }
        }
    });

    // Load data provinsi
    $.ajax({
        url: 'https://www.emsifa.com/api-wilayah-indonesia/api/provinces.json',
        type: 'GET',
        success: function(provinces) {
            $('#provinceSelect').empty().append('<option value="">Pilih Provinsi</option>');
            provinces.forEach(function(province) {
                $('#provinceSelect').append(`<option value="${province.id}">${province.name}</option>`);
            });

            // Jika ada alamat lengkap, coba cari dan pilih provinsi yang sesuai
            if (fullAddress) {
                provinces.forEach(function(province) {
                    if (fullAddress.includes(province.name)) {
                        $('#provinceSelect').val(province.id).trigger('change');
                        loadCities(province.id, fullAddress);
                    }
                });
            }
        }
    });

    // Fungsi untuk load data kota
    function loadCities(provinceId, fullAddress = '') {
        $.ajax({
            url: `https://www.emsifa.com/api-wilayah-indonesia/api/regencies/${provinceId}.json`,
            type: 'GET',
            success: function(cities) {
                $('#citySelect').empty().append('<option value="">Pilih Kabupaten/Kota</option>').prop('disabled', false);
                cities.forEach(function(city) {
                    $('#citySelect').append(`<option value="${city.id}">${city.name}</option>`);
                });

                // Jika ada alamat lengkap, coba cari dan pilih kota yang sesuai
                if (fullAddress) {
                    cities.forEach(function(city) {
                        if (fullAddress.includes(city.name)) {
                            $('#citySelect').val(city.id).trigger('change');
                            loadDistricts(city.id, fullAddress);
                        }
                    });
                }
            }
        });
    }

    // Fungsi untuk load data kecamatan
    function loadDistricts(cityId, fullAddress = '') {
        $.ajax({
            url: `https://www.emsifa.com/api-wilayah-indonesia/api/districts/${cityId}.json`,
            type: 'GET',
            success: function(districts) {
                $('#districtSelect').empty().append('<option value="">Pilih Kecamatan</option>').prop('disabled', false);
                districts.forEach(function(district) {
                    $('#districtSelect').append(`<option value="${district.id}">${district.name}</option>`);
                });

                // Jika ada alamat lengkap, coba cari dan pilih kecamatan yang sesuai
                if (fullAddress) {
                    districts.forEach(function(district) {
                        if (fullAddress.includes(`Kec. ${district.name}`)) {
                            $('#districtSelect').val(district.id).trigger('change');
                            loadVillages(district.id, fullAddress);
                        }
                    });
                }
            }
        });
    }

    // Fungsi untuk load data desa
    function loadVillages(districtId, fullAddress = '') {
        $.ajax({
            url: `https://www.emsifa.com/api-wilayah-indonesia/api/villages/${districtId}.json`,
            type: 'GET',
            success: function(villages) {
                $('#villageSelect').empty().append('<option value="">Pilih Desa/Kelurahan</option>').prop('disabled', false);
                villages.forEach(function(village) {
                    $('#villageSelect').append(`<option value="${village.id}">${village.name}</option>`);
                });

                // Jika ada alamat lengkap, coba cari dan pilih desa yang sesuai
                if (fullAddress) {
                    villages.forEach(function(village) {
                        if (fullAddress.includes(`Desa/Kel. ${village.name}`)) {
                            $('#villageSelect').val(village.id).trigger('change');
                        }
                    });
                }
            }
        });
    }

    // Event handler untuk perubahan provinsi
    $('#provinceSelect').on('change', function() {
        const provinceId = $(this).val();
        $('#citySelect').empty().prop('disabled', true);
        $('#districtSelect').empty().prop('disabled', true);
        $('#villageSelect').empty().prop('disabled', true);

        if (provinceId) {
            loadCities(provinceId);
        }
        updateFullAddress();
    });

    // Event handler untuk perubahan kota
    $('#citySelect').on('change', function() {
        const cityId = $(this).val();
        $('#districtSelect').empty().prop('disabled', true);
        $('#villageSelect').empty().prop('disabled', true);

        if (cityId) {
            loadDistricts(cityId);
        }
        updateFullAddress();
    });

    // Event handler untuk perubahan kecamatan
    $('#districtSelect').on('change', function() {
        const districtId = $(this).val();
        $('#villageSelect').empty().prop('disabled', true);

        if (districtId) {
            loadVillages(districtId);
        }
        updateFullAddress();
    });

    // Event handler untuk perubahan desa
    $('#villageSelect').on('change', function() {
        updateFullAddress();
    });

    // Format RT/RW saat input
    $('#address_rt, #address_rw').on('input', function() {
        // Hapus karakter non-angka
        let value = this.value.replace(/[^0-9]/g, '');

        // Batasi panjang ke 3 digit
        if (value.length > 3) {
            value = value.slice(0, 3);
        }

        // Tambahkan leading zeros jika panjang kurang dari 3
        if (value.length > 0) {
            value = value.padStart(3, '0');
        }

        // Update nilai input
        this.value = value;

        // Update alamat lengkap
        updateFullAddress();
    });

    // Format RT/RW saat fokus hilang (blur)
    $('#address_rt, #address_rw').on('blur', function() {
        let value = this.value.trim();
        if (value.length > 0) {
            // Pastikan selalu 3 digit dengan leading zeros
            this.value = value.padStart(3, '0');
        }
    });

    // Format RT/RW saat fokus (focus)
    $('#address_rt, #address_rw').on('focus', function() {
        let value = this.value.trim();
        // Hapus leading zeros saat input mendapat fokus
        if (value.length > 0) {
            this.value = parseInt(value, 10).toString();
        }
    });

    // Format kode pos
    $('#postal_code').on('input', function() {
        // Hapus karakter non-angka
        let value = this.value.replace(/[^0-9]/g, '');

        // Batasi panjang ke 5 digit
        if (value.length > 5) {
            value = value.slice(0, 5);
        }

        // Update nilai input
        this.value = value;
    });

    // Validasi kode pos saat form disubmit
    $('form').on('submit', function(e) {
        const postalCode = $('#postal_code').val();
        if (postalCode.length !== 5) {
            e.preventDefault();
            alert('Kode pos harus 5 digit angka');
            $('#postal_code').focus();
            return false;
        }
    });

    // Update alamat lengkap
    function updateFullAddress() {
        const street = $('#address_street').val().trim();
        const rt = $('#address_rt').val().trim();
        const rw = $('#address_rw').val().trim();
        const postalCode = $('#postal_code').val().trim();
        const village = $('#villageSelect option:selected').text();
        const district = $('#districtSelect option:selected').text();
        const city = $('#citySelect option:selected').text();
        const province = $('#provinceSelect option:selected').text();

        const addressParts = [];

        // Tambahkan alamat jalan jika ada
        if (street) {
            addressParts.push(street);
        }

        // Tambahkan RT/RW jika ada
        if (rt || rw) {
            const formattedRt = rt ? rt.padStart(3, '0') : '-';
            const formattedRw = rw ? rw.padStart(3, '0') : '-';
            addressParts.push(`RT ${formattedRt}/RW ${formattedRw}`);
        }

        // Tambahkan Desa/Kelurahan jika dipilih
        if (village && village !== 'Pilih Desa/Kelurahan') {
            addressParts.push(`Desa/Kel. ${village}`);
        }

        // Tambahkan Kecamatan jika dipilih
        if (district && district !== 'Pilih Kecamatan') {
            addressParts.push(`Kec. ${district}`);
        }

        // Tambahkan Kabupaten/Kota jika dipilih
        if (city && city !== 'Pilih Kabupaten/Kota') {
            addressParts.push(city);
        }

        // Tambahkan Provinsi jika dipilih
        if (province && province !== 'Pilih Provinsi') {
            addressParts.push(province);
        }

        // Tambahkan Kode Pos jika ada
        if (postalCode) {
            addressParts.push(postalCode);
        }

        // Update hidden input untuk alamat lengkap
        $('#fullAddress').val(addressParts.join(', '));
    }

    // Panggil updateFullAddress saat form di-submit
    $('#patientForm').on('submit', function(e) {
        updateFullAddress();
        return true;
    });

    // Tambahkan event listener untuk setiap perubahan komponen alamat
    $('#address_street, #address_rt, #address_rw, #postal_code').on('input', function() {
        updateFullAddress();
    });

    $('#provinceSelect, #citySelect, #districtSelect, #villageSelect').on('change', function() {
        updateFullAddress();
    });
});
</script>
@endpush

@push('styles')
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
<style>
    /* Custom style untuk Select2 */
    .select2-container--classic {
        width: 100% !important;
    }

    .select2-container--classic .select2-selection--single {
        height: 48px !important;
        border: 1px solid rgb(209, 213, 219) !important;
        border-radius: 0.5rem !important;
        background: white !important;
        display: flex !important;
        align-items: center !important;
    }

    .select2-container--classic .select2-selection--single .select2-selection__rendered {
        color: rgb(55, 65, 81) !important;
        line-height: 48px !important;
        padding-left: 1rem !important;
        padding-right: 2.5rem !important;
        font-size: 0.875rem !important;
    }

    .select2-container--classic .select2-selection--single .select2-selection__arrow {
        height: 46px !important;
        width: 40px !important;
        border-left: none !important;
        border-radius: 0 0.5rem 0.5rem 0 !important;
        background: transparent !important;
    }

    .select2-container--classic .select2-selection--single .select2-selection__arrow b {
        border-color: #6B7280 transparent transparent transparent !important;
    }

    .select2-container--classic.select2-container--open .select2-selection--single .select2-selection__arrow b {
        border-color: transparent transparent #6B7280 transparent !important;
    }

    .select2-container--classic .select2-search--dropdown {
        padding: 0.5rem !important;
        background: white !important;
    }

    .select2-container--classic .select2-search--dropdown .select2-search__field {
        border: 1px solid rgb(209, 213, 219) !important;
        border-radius: 0.375rem !important;
        padding: 0.5rem 1rem !important;
        font-size: 0.875rem !important;
    }

    .select2-container--classic .select2-results__option {
        padding: 0.75rem 1rem !important;
        font-size: 0.875rem !important;
        transition: all 0.2s !important;
    }

    .select2-container--classic .select2-results__option--highlighted[aria-selected] {
        background-color: rgb(59, 130, 246) !important;
    }

    .select2-container--classic.select2-container--open .select2-dropdown {
        border-color: rgb(59, 130, 246) !important;
        border-radius: 0.5rem !important;
        box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06) !important;
        margin-top: 4px !important;
    }

    .select2-container--classic .select2-results__option[aria-disabled=true] {
        color: rgb(156, 163, 175) !important;
        background-color: rgb(249, 250, 251) !important;
    }

    .select2-container--classic.select2-container--disabled .select2-selection--single {
        background-color: rgb(249, 250, 251) !important;
        cursor: not-allowed !important;
    }

    /* Loading state */
    .select2-container--classic .select2-results__option.loading-results {
        padding: 1rem !important;
        text-align: center !important;
        color: rgb(107, 114, 128) !important;
    }

    /* Empty state */
    .select2-container--classic .select2-results__option.select2-results__message {
        padding: 1rem !important;
        text-align: center !important;
        color: rgb(107, 114, 128) !important;
        font-style: italic !important;
    }
</style>
@endpush

@endsection
