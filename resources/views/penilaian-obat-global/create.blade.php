...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Tambah Penilaian Obat Global</h1>
        <form action="{{ route('penilaian-obat-global.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="nilai_l" id="nilai_l" class="mt-1 block w-full rounded-md border-gray-300" value="{{ old('nilai_l') }}" required min="1" max="10">
                    @error('nilai_l')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="nilai_m" id="nilai_m" class="mt-1 block w-full rounded-md border-gray-300" value="{{ old('nilai_m') }}" required min="1" max="10">
                    @error('nilai_m')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="nilai_u" id="nilai_u" class="mt-1 block w-full rounded-md border-gray-300" value="{{ old('nilai_u') }}" required min="1" max="10">
                    @error('nilai_u')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
