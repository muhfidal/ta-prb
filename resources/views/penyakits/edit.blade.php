...
<div class="container mx-auto px-4 py-10">
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-10 max-w-xl mx-auto">
        <h1 class="text-2xl font-bold text-blue-800 flex items-center gap-2 mb-8">
            ...
            Edit Penyakit (Gejala Tambahan)
        </h1>
        <form action="{{ route('penyakits.update', $penyakit->id) }}" method="POST" autocomplete="off">
            @csrf
            @method('PUT')
            <div class="mb-5">
                <label for="nama_penyakit" class="block mb-2 font-semibold text-gray-700">Nama Penyakit</label>
                <input type="text" name="nama_penyakit" id="nama_penyakit" value="{{ old('nama_penyakit', $penyakit->nama_penyakit) }}" class="form-input w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>
                @error('nama_penyakit')
